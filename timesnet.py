...
from neuralforecast.common._modules import DataEmbedding, TimeFeatureEmbedding
...
class TimesNet(BaseWindows):
    ...
    **trainer_kwargs
    ...
    def __init__(
        ...
        **trainer_kwargs
        ...
        self.enc_embedding = DataEmbedding(
            c_in=self.enc_in,
            exog_hist_input_size=self.hist_input_size,
            exog_futr_input_size=self.futr_input_size,
            hidden_size=hidden_size,
        ...
        if self.futr_input_size > 0:
            self.temporal_embedding_futr = TimeFeatureEmbedding(
                input_size=self.futr_input_size, hidden_size=hidden_size
            )
            self.predict_linear_futr = nn.Linear(self.h, self.h)
        else:
            self.temporal_embedding_futr = None
            self.predict_linear_futr = None
    ...
    def forward(self, windows_batch):
        ...
        if self.hist_input_size > 0:
            x_mark_hist = futr_exog[:, : self.input_size, :]
        else:
            x_mark_hist = None
        if self.futr_input_size > 0:
            x_mark_futr = futr_exog[:, : self.input_size, :]
            x_futr = futr_exog[:, self.input_size : , :]
        else:
            x_mark_futr = None
            x_futr = None

        # embedding
        enc_out = self.enc_embedding(insample_y, x_mark_hist, x_mark_futr)
        enc_out = self.predict_linear(enc_out.permute(0, 2, 1)).permute(
            0, 2, 1
        )  # align temporal dimension
        if x_futr is not None:
            enc_futr = self.temporal_embedding_futr(x_futr)
            enc_futr = self.predict_linear_futr(enc_futr.permute(0, 2, 1)).permute(
            0, 2, 1
        )
            enc_futr = F.pad(input=enc_futr, pad=(0, 0, self.input_size, 0), mode='constant', value=0)
