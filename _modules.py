...
class DataEmbedding(nn.Module):
    def __init__(
        self, c_in, exog_hist_input_size, exog_futr_input_size, hidden_size, pos_embedding=True, dropout=0.1
    ):
        ...
        if exog_hist_input_size > 0:
            self.temporal_embedding_hist = TimeFeatureEmbedding(
                input_size=exog_hist_input_size, hidden_size=hidden_size
            )
        else:
            self.temporal_embedding_hist = None

        if exog_futr_input_size > 0:
            self.temporal_embedding_futr = TimeFeatureEmbedding(
                input_size=exog_futr_input_size, hidden_size=hidden_size
            )
        else:
    ...
    def forward(self, x, x_mark_hist=None, x_mark_futr=None):
        ...
        if self.temporal_embedding_hist is not None:
            x = x + self.temporal_embedding_hist(x_mark_hist)

        if self.temporal_embedding_futr is not None:
            x = x + self.temporal_embedding_futr(x_mark_futr)
